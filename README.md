...
- **Security, defense in depth:**
  ...
  - *Destructive-intent guard (no silent rewriting)* — before any model code is generated, a deterministic check scans the ticket's own text for destructive/DDL intent (dropping/truncating/altering a table, schema, or database). If found, the ticket is rejected with an explanatory Jira comment and routed to a human — the agent never silently "launders" a destructive request into a harmless SELECT that fails to do what the user asked. Transparency over quiet substitution.
